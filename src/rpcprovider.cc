...
{
    //静态成员函数可以通过作用域直接调用
    //MprpcApplication::GetInstance()会生成一个MprpcApplication类
    //在MprpcApplication中，GetInstance()静态方法返回的是这个唯一实例的引用（或指针）。
    //这样，无论在哪里调用GetInstance()，得到的都是同一个对象。
    //MprpcApplication::GetInstance().GetConfig()会返回唯一MprpcApplication类中的MprpcConfig类
    //（该类在init中已经被初始化），再用Load就可以获得对应的value值，一个string类型的地址
    std::string ip=MprpcApplication::GetInstance().GetConfig().Load("rpcserverip");
    //注意到address中port是uint16_t格式，而存在m_config中的m_configMap是<sting,sting>，因此需要格式转换
    uint16_t port=atoi(MprpcApplication::GetInstance().GetConfig().Load("rpcserverport").c_str());
    muduo::net::InetAddress address(ip,port,false);
    //在定义中为InetAddress(StringArg ip, uint16_t port, bool ipv6 = false);
...
*/
    ...
    std::cout<<"test: rpcprovider.cc::run()"<<"\n"
    <<"RpcProvider start service at ip:"<<ip<<" port"<<port<<std::endl;
                            ...
                            muduo::Timestamp)
{
    ...
    //这里是通过protobuf序列化得到的，不需要管网络字节序和主机字节序
